...
std::optional<std::string> get_special_key(std::string_view key,
  ...
  if (key == "Background" || key == "Video") {
    ...
    if (key == "Video") {
      if (get_video_extensions().count(ext_lower) == 0) {
        return std::nullopt;
      }
    } else {
      if (get_image_extensions().count(ext_lower) == 0) {
        return std::nullopt;
      }
    }
...
std::string osu_parser::get_property(std::string_view content,
                                     std::string_view key) {
  ...
  if (start < content.size()) {
    ...
    if (delimiter_i != std::string_view::npos) {
      std::string_view current_key =
          trim_view(line_view.substr(0, delimiter_i));
      if (current_key == key) {
        std::string_view value = trim_view(line_view.substr(delimiter_i + 1));
        return std::string(value);
      }
...
osu_parser::get_properties(std::string_view content,
  ...
  const auto &key_to_section_global = get_key_to_section();
  const auto &special_keys_global = get_special_keys();

  for (const auto &key : keys) {
    keys_to_find.insert(key);
    if (key_to_section_global.count(key)) {
      key_to_section_map[key] = key_to_section_global.at(key);
    }
  }
  ...
  auto process_line = [&](std::string_view line_view) {
    ...
    if (delimiter_i != std::string_view::npos) {
      ...
      if (keys_to_find.count(current_key)) {
        bool correct_section =
            !key_to_section_map.count(current_key) ||
            key_to_section_map.at(current_key) == current_section;

        bool not_found_yet =
            results.find(std::string(current_key)) == results.end();
    ...
    for (const auto &key : keys) {
      if (!special_keys_global.count(key)) {
        continue;
      }
      ...
      if (key_to_section_map.count(key) &&
          key_to_section_map.at(key) == current_section) {
        auto result = get_special_key(key, line_view);
        if (result.has_value()) {
